    ...
    public List<RoomBean> searchRoomCondition(Date startDate, Date endDate, Integer size) {
        try {
            ...
            for(int i = 0; i<listRooms.size(); ++i) {
                RoomBean roomBean = new RoomBean();
                roomBean = getRoomById(Integer.parseInt(listRooms.get(i)+""));
                SimpleDateFormat sdf = new SimpleDateFormat("yyyy/MM/dd");
                roomBean.setStart(sdf.format(startDate));
                roomBean.setEnd(sdf.format(endDate));
                listRoomsBean.add(roomBean);
            }
            System.out.println("----listRoomsBean size : "+listRoomsBean.size());
            return listRoomsBean;
        } catch (Exception e) {
